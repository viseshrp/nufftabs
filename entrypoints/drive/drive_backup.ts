  ...
  const entries = files
    ...
    .map((file) => {
      ...
      return {
        ...
        tabGroupCount: extractTabGroupCountFromFileName(typeof file.name === 'string' ? file.name : ''),
      };
    });
...
export async function readRetentionCount(): Promise<number> {
  const raw = await chrome.storage.local.get([DRIVE_STORAGE_KEYS.retentionCount]);
  return normalizeRetentionCount(raw[DRIVE_STORAGE_KEYS.retentionCount], DEFAULT_RETENTION_COUNT);
}

/** Writes normalized retention count to storage. */
export async function writeRetentionCount(retentionCount: number): Promise<number> {
  const normalized = normalizeRetentionCount(retentionCount, DEFAULT_RETENTION_COUNT);
  await chrome.storage.local.set({ [DRIVE_STORAGE_KEYS.retentionCount]: normalized });
  return normalized;
}
...
export async function readLocalIndex(): Promise<DriveBackupIndex> {
  const raw = await chrome.storage.local.get([DRIVE_STORAGE_KEYS.driveBackupIndex, DRIVE_STORAGE_KEYS.installId]);
  ...
  if (typeof maybeInstallId !== 'string' || !Array.isArray(maybeBackups)) return fallback;

  const normalizedBackups = maybeBackups
    .filter((entry): entry is DriveBackupEntry => Boolean(entry && typeof entry === 'object'))
    .map((entry) => {
      const fileId = typeof entry.fileId === 'string' ? entry.fileId : '';
      const fileName = typeof entry.fileName === 'string' ? entry.fileName : 'backup.json';
      const timestamp =
      ...
      const tabGroupCount =
        typeof entry.tabGroupCount === 'number' && Number.isFinite(entry.tabGroupCount)
...
export async function updateLocalIndex(installId: string, backups: DriveBackupEntry[]): Promise<void> {
  ...
  await chrome.storage.local.set({ [DRIVE_STORAGE_KEYS.driveBackupIndex]: next });
}
...
export async function enforceRetention(
  ...
  const normalizedRetention = normalizeRetentionCount(retentionCount, DEFAULT_RETENTION_COUNT);
 ...
 */
export async function listDriveBackups(token: string, deps: DriveApiDeps = defaultDeps): Promise<DriveBackupEntry[]> {
...
export async function performBackup(
  ...
  const retention =
    typeof requestedRetentionCount === 'number'
      ? normalizeRetentionCount(requestedRetentionCount, DEFAULT_RETENTION_COUNT)
      : await readRetentionCount();
  const backups = await enforceRetention(installFolderId, retention, token, deps);
...
export async function restoreFromBackup(
  ...
  const restoredTabs = Object.values(groups).reduce((total, tabs) => total + tabs.length, 0);
...
export async function getBackupsWithFallback(
  ...
  try {
    ...
    logExtensionError('Failed to list Drive backups as fallback', error, { operation: 'runtime_context' });
    return [];
  }
}
